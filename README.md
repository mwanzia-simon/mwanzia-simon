<h1 align="center">Hi 👋, I'm Simon</h1>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=MERN+Stack+Developer;Full+Stack+Web+Developer;IT+Student;Building+Real+World+Applications;Always+Learning+New+Things&center=true&width=550&height=50">
</p>

---

## 🧑‍💻 About Me

* 🎓 IT Student passionate about software development
* 💻 **MERN Stack Developer** focused on building full-stack web applications
* ⚛️ Building modern and responsive interfaces with **React**
* ⚙️ Developing backend APIs and services with **Node.js & Express**
* 🍃 Working with **MongoDB** for database-driven applications
* 🔐 Interested in authentication, authorization, APIs, and backend architecture
* 🏗️ Currently building and improving real-world projects
* 🧠 Constantly learning, experimenting, and improving my development skills
* ⚡ I enjoy leveling up like in anime

---

## 🚀 Tech Stack

### 🌐 Frontend

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,react" />
</p>

* HTML5
* CSS3
* JavaScript
* React
* Responsive Web Design

### ⚙️ Backend

<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,python,flask" />
</p>

* Node.js
* Express.js
* REST APIs
* Python
* Flask
* Authentication & Authorization
* Backend Architecture

### 🗄️ Databases

<p>
  <img src="https://skillicons.dev/icons?i=mongodb,mysql,sqlite" />
</p>

* MongoDB
* MySQL
* SQLite
* Database Design

### 🛠️ Tools & Technologies

<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,postman,npm" />
</p>

* Git & GitHub
* VS Code
* Postman
* npm
* RESTful APIs

---

## 📌 What I Build

I enjoy building full-stack applications that combine a modern frontend with a structured and scalable backend.

Some areas I work with include:

* 🔐 User authentication & authorization
* 👤 User and account management
* 🛒 E-commerce functionality
* 📦 Product & inventory management
* 💳 Payment integrations
* 📧 Email services and verification systems
* 🔌 RESTful APIs
* 🗄️ Database-driven applications
* 📊 Admin dashboards
* 🏗️ Service-based backend architecture

---

## 🔥 Current Focus

I'm currently focused on improving my skills as a **MERN Stack Developer**, with particular interest in:

* Backend architecture
* Scalable APIs
* Authentication & security
* Database design
* Third-party API integrations
* Clean and maintainable code
* Building complete production-style applications

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=mwanzia-simon&show_icons=true&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=mwanzia-simon&theme=radical" />
</p>

---

## 📊 Top Languages

<p align="center">
  <img src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=mwanzia-simon&layout=compact&langs_count=8&theme=algolia" />
</p>

---

## 🌍 Connect With Me

* 💻 GitHub: https://github.com/mwanzia-simon

---

## 💡 Quote

> "Consistency beats motivation."

---

<p align="center">
  ⭐ Thanks for visiting my profile!
</p>
